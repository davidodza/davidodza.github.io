...
I'm going to use Burp Suite for account harvesting on the Wordpress login.

First I'll send a Request to the web application with Username = "Test" and Password = "Test". 

![BurpSuite (For crafting the python username enumerater).PNG]({{site.baseurl}}/images/MrRobot/BurpSuite (For crafting the python username enumerater).PNG)

Next, I find the section in the web application's Response that has the descriptive error message. That will be the section of the Response that I'm counting on to be different for valid and invalid usernames.

![burpResponse.PNG]({{site.baseurl}}/images/MrRobot/burpResponse.PNG)

Now that I know where the username and password go in the Request, and the error messages shows up in the Response, i'm ready to send the request to the Burp Suite Intruder.

In the screenshot below, I'm indicating to Intruder that I want the username in the Request to change. I'm planning on running all the lines from the dictionary I got earlier as usernames.

![sentRequestToIntruder.PNG]({{site.baseurl}}/images/MrRobot/sentRequestToIntruder.PNG)
